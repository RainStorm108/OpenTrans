...
from opentrans import config

logger = logging.getLogger(__name__)

@pytest.fixture
def tmp_settings(tmp_path) -> config.Settings:
  in_dir = tmp_path / "in"
  ...
  return config.Settings(
    ...
    hash_algo='sha1',
    translate_file_types = set(['.md', '.mdx', '.txt'])
  )

@pytest.fixture
def tmp_translator(tmp_settings: config.Settings):
  return Translator(cache_path=tmp_settings.cache_path)

@pytest.fixture
def tmp_file(tmp_settings: config.Settings) -> Path:
  p = tmp_settings.input_dir / "test.md"
  p.write_text("This is a test for doctrans. What is the meaning of life?", encoding="utf-8")
  return p

def test_translate(tmp_file: Path, tmp_settings: config.Settings, tmp_translator: Translator):
...
@pytest.fixture
def complex_md_file(tmp_settings: config.Settings) -> Path:
...
def test_translate_comprehensive(complex_md_file, tmp_settings, expected_keyword, target_lang, tmp_translator):
  config.settings = tmp_settings
